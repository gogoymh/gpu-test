...
model_name = os.path.join("C://유민형//개인 연구//gpu_test//", "resnet56.pth")
...
for i in range(9):
    print("-"*10)
    a = np.sqrt(model.layer1[i].bn1.weight.detach().numpy())
    b = np.sqrt(model.layer1[i].bn1.running_var.detach().numpy())
    c = a/b
    print(c.min(), c.max())

    d = np.sqrt(model.layer1[i].bn2.weight.detach().numpy())
...
for i in range(9):
    print("-"*10)
    a = np.sqrt(model.layer2[i].bn1.weight.detach().numpy())
    b = np.sqrt(model.layer2[i].bn1.running_var.detach().numpy())
    c = a/b
    print(c.min(), c.max())

    d = np.sqrt(model.layer2[i].bn2.weight.detach().numpy())
...
for i in range(9):
    print("-"*10)
    a = np.sqrt(model.layer3[i].bn1.weight.detach().numpy())
    b = np.sqrt(model.layer3[i].bn1.running_var.detach().numpy())
    c = a/b
    print(c.min(), c.max())

    d = np.sqrt(model.layer3[i].bn2.weight.detach().numpy())
