...
class My_Conv2d(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=3, stride=1, padding=0, groups=1, dilation=1, bias=True):
        ...
        self.weight1 = nn.Parameter(weight, requires_grad=False)
        self.weight2 = nn.Parameter(weight)
    ...
    def forward(self, input):
        if self.training:
            ...
            out = F.conv2d(input,self.weight1,bias=None,stride=self.stride,padding=self.padding)
        return out
...
class My_norm2d(nn.Module):
    def __init__(self, out_channel):
        ...
        scale = 0.1
        self.alpha = nn.Parameter(torch.ones(1,out_channel,1,1)*scale, requires_grad=False)
    ...
    def forward(self, input):
        if self.training:            
            ...
            out = self.alpha * input + beta 
            
            self.beta1.data = beta.data
            
        else:
            out = self.alpha * input + self.beta1
