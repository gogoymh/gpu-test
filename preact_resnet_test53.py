...
class ECA_sigmoid(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            mean = (1-self.delta)*self.mean + self.delta*self.avg_pool(x)
            gate = self.conv(mean.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
            gate = self.sigmoid(gate) * self.scale
        
            out = gate * (x - mean) + self.bias
            
            self.mean.data = mean.mean(dim=0, keepdim=True).data

        else:
            gate = self.conv(self.mean.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
            gate = self.sigmoid(gate)
            out = gate * (x - self.mean) + self.bias
        return out
