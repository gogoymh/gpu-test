...
class My_norm2d(nn.Module):
    def __init__(self, out_channel):
        ...
        self.alpha = nn.Parameter(torch.ones(1,out_channel,1,1)/math.sqrt(out_channel))
        self.beta = nn.Parameter(torch.randn(1,out_channel,1,1))
    ...
    def forward(self, input):
        ...
        if self.training:
            x = self.cnt // 391
            std = self.base * (0.95 ** (x))
            alpha = alpha + torch.normal(0, std, size=(1,self.out_channel,1,1)).cuda() + 1e-6
            beta = beta + torch.normal(0, std, size=(1,self.out_channel,1,1)).cuda()
            self.cnt += 1
            #alpha = alpha + torch.normal(0, 0.001, size=(1,self.out_channel,1,1)).cuda()
            #beta = beta + torch.normal(0, 0.001, size=(1,self.out_channel,1,1)).cuda()
            
        out = input * alpha.clamp(0,0.1)
        bias = self.bias - alpha.clamp(0,0.1) * beta
        out = (out + bias) * self.scale.clamp(0,1)
        return out
