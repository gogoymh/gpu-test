...
class My_norm2d(nn.Module):
    def __init__(self, out_channel):
        ...
        self.gamma1 = nn.Parameter(torch.zeros(1,out_channel,1,1), requires_grad=False)
        self.gamma2 = nn.Parameter(torch.zeros(1,out_channel,1,1))
        
        
        #self.scale1 = nn.Parameter(torch.ones(1)*scale, requires_grad=False)
        #self.scale2 = nn.Parameter(torch.ones(1)*scale)
        #self.scale = 2
        #self.scale = 0.5 - 1/math.sqrt(out_channel)
        #self.scale = 1/math.sqrt(out_channel)
        self.delta = 0.01
    ...
    def forward(self, input):            
        if self.training:
            ...
            beta = (1-self.delta)*self.beta1 + self.delta*self.beta2
            gamma = (1-self.delta)*self.gamma1 + self.delta*self.gamma2
            #scale = (1-self.delta)*self.scale1 + self.delta*self.scale2
            
            out = alpha * (input - beta) + gamma
            #out = out * scale#.clamp(1e-6,1)
            
            self.alpha1.data = alpha.data
            self.beta1.data = beta.data
            self.gamma1.data = gamma.data
            #self.scale1.data = scale.data
            
        else:
            out = self.alpha1 * (input - self.beta1) + self.gamma1
            #out = out * self.scale1#.clamp(1e-6,1)
