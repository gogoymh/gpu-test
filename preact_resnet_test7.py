...
class My_norm2d(nn.Module):
    def __init__(self, out_channel):
        ...
        self.delta_min = 0.01
        self.delta_max = 0.11
        self.delta_scale = 0.02
        self.half_cycle = 5
        self.current = 0
        
    def forward(self, input):            
        if self.training:            
            if self.current < self.half_cycle:
                delta = self.delta_min + self.delta_scale * (self.current % self.half_cycle)
            else:
                delta = self.delta_max - self.delta_scale * (self.current % self.half_cycle)
                            
            alpha = (1-delta)*self.alpha1 + delta*self.alpha2
            beta = (1-delta)*self.beta1 + delta*self.beta2
