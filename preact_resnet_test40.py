...
class My_norm2d(nn.Module):
    def __init__(self, out_channel):
        ...
        #self.gamma = ECA_Layer(out_channel)
        self.bias1 = nn.Parameter(torch.zeros(1,out_channel,1,1), requires_grad=False)
        self.bias2 = nn.Parameter(torch.zeros(1,out_channel,1,1))
        
        self.delta = 0.01
        
    def forward(self, input):
        if self.training:
            bias = (1-self.delta)*self.bias1 + self.delta*self.bias2
            out = self.alpha(input)
            out = self.beta(out) + bias
            
            self.bias1.data = bias.data
        
        else:
            out = self.alpha(input)
            out = self.beta(out) + self.bias1
