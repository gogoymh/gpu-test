...
class My_norm2d(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            x = (self.cnt // 85) + 1
            std = (1/math.sqrt(self.batch_size-1))/math.exp((x-1)/2)
            alpha = alpha + torch.normal(0, std, size=(1,self.out_channel,1,1)).cuda()
            beta = beta + torch.normal(0, std, size=(1,self.out_channel,1,1)).cuda()
            self.cnt += 1
        out = input * alpha.clamp(0,1.5)
        bias = self.bias.clamp(-1.5,1.5) - alpha.clamp(0,1.5) * beta
        out = (out + bias) * self.scale.clamp(-1.5,1.5)
        return out
