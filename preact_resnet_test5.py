...
class My_norm2d(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            C = input.shape[0]
            x = self.cnt // 391
            std1 = self.base1 * (0.95 ** (x))
            std2 = self.base2 * (0.95 ** (x))
            alpha = alpha + torch.normal(0, std1, size=(C,self.out_channel,1,1)).cuda()
            beta = beta + torch.normal(0, std2, size=(C,self.out_channel,1,1)).cuda()
            self.cnt += 1
            
        out = input * alpha.clamp(0,0.1)
        bias = self.bias - alpha.clamp(0,0.1) * beta
        out = (out + bias) * self.scale.clamp(min=0)#.clamp(0,1)
