...
class Attention(nn.Module):
    def __init__(self, channel, kernel=3):
        ...
        scale = 0.5 - 1/math.sqrt(channel)
        self.scale = nn.Parameter(torch.ones(1,channel)*scale)
        self.shift = nn.Parameter(torch.zeros(1,channel))
    ...
    def forward(self, x):
        ...
        x = self.tanh(x) * self.scale + self.shift# + torch.normal(0, std, size=(1,1)).cuda()
...
class My_norm2d(nn.Module):
    def __init__(self, out_channel):
        ...
        self.alpha = Attention(out_channel)
        self.beta = Attention(out_channel)
        self.gamma = nn.Parameter(torch.zeros(1,out_channel,1,1))
        
        scale = 0.5 - 1/math.sqrt(out_channel)
        self.scale = nn.Parameter(torch.ones(1,out_channel,1,1)*scale)
        
    def forward(self, x):
        alpha = self.alpha(x)
        beta = self.beta(x)
        out = (alpha * x - alpha * beta + self.gamma) * self.scale
        return out
